...
class CalendarController {
     ...
     */
    listCal(userID, syncedOnly = false, importCal = true) {
        return new Promise((resolve, reject) => {
            ...
            new Promise((innerResolve, innerReject) => {
                if (importCal) {
                    Calendar.find(calendarFilter)
                        .then(calendars => {
                            ...
                            this.importCalToMongo(userID, syncToken)
                ...
                } else innerResolve("Not importing")
            })
                .finally(() => {
     ...
     */
    importCalToMongo(userID, storedSyncToken = null) {
        return new Promise((resolve, reject) => {
            this.gcalController.listCal(storedSyncToken)
                .then((calendarListResult) => {
                    let calendarList = calendarListResult.data;

                    // If calendars exist
                    if (calendarList.items.length > 0) {
                        Promise.all(this.handleCalendarList(userID, calendarList, storedSyncToken))
                ...
                .catch(err => {
                    ...
                    * Perform a full sync!
                    */
                    this.gcalController.listCal().then((calendarListResult) => {
                        Promise.all(this.handleCalendarList(userID, calendarListResult.data))
                            .then(calendars => {
                                resolve(calendars);
                            })
                            .catch(err => {
     ...
     * @param {String} storedSyncToken
     */
    handleCalendarList(userID, calendarList, storedSyncToken) {
     ...
     */
    saveSyncToken(googleID, nextSyncToken) {
        return Calendar.findOneAndUpdate(
            { googleID },
            {
